...
function ChoroplethMap(props) {
  ...
  const mapData = selector => {
    ...
    const color = d3.scaleSequential(d3.interpolateReds).domain([0, statistic.maxConfirmed / maxInterpolation]);
    ...
    function ready([india]) {
      ...
      svg
        ...
        .text(function(d) {
          return (
            parseFloat(100 * (unemployment.get(d.properties.ST_NM.toLowerCase()) / statistic.total)).toFixed(2) +
            '% from ' +
